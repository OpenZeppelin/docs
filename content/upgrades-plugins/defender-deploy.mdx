...
<Callout>
The API key for the above must at least have the capability to Manage Deployments (optionally Manage Relayers is needed to create an approval process with a Relayer). You can configure your API keys at https://defender.openzeppelin.com/#/settings/api-keys.
</Callout>
...
When using the [Hardhat Upgrades API functions](/upgrades-plugins/api-hardhat-upgrades), enable OpenZeppelin Defender deployments using any of the ways below.
