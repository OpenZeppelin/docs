...
This package adds functions to your Hardhat scripts so you can deploy and upgrade proxies for your contracts. It offers two APIs that you can choose between: an ethers-based API (the default, imported from `@openzeppelin/hardhat-upgrades`) and a viem-based API (imported from `@openzeppelin/hardhat-upgrades/viem`).
...
Then install the peer dependencies for the API you use. For the ethers-based API:

```console
$ npm install --save-dev @nomicfoundation/hardhat-ethers ethers
```

For the viem-based API:
...
<Callout>
`@nomicfoundation/hardhat-ethers` and `ethers` are optional peer dependencies, required only by the ethers-based API; projects that use only the viem-based API do not need them. (Before this version they were required peer dependencies, so package managers installed them automatically.)
</Callout>
...
<Callout>
This default import registers the plugin and works with either API. If your project uses only the viem-based API and does not install `@nomicfoundation/hardhat-ethers`, import the plugin from `@openzeppelin/hardhat-upgrades/viem` instead — it is the same plugin, re-exported so it can be registered without referencing the ethers types. An `@openzeppelin/hardhat-upgrades/ethers` alias is also available as an alternative to the default import.
</Callout>
...
Both APIs accept the same options and run the same upgrade-safety validations. A couple of behaviors differ:

- **Overloaded function names.** When you pass a bare function name (no parameter types) for the `initializer` option or an upgrade `call`, and the contract has multiple overloads of that name, the viem-based API resolves the overload from the argument types, while the ethers-based API requires the full function signature in that case.
- **Function reference formats.** For the `initializer` option and upgrade `call`, the viem-based API accepts a function name or a canonical signature such as `initialize(uint256)`. It does not accept a 4-byte selector or non-canonical signature spellings that the ethers-based API tolerates.
