...
<Callout>
`@nomicfoundation/hardhat-ethers`/`ethers` and `@nomicfoundation/hardhat-viem`/`viem` are optional peer dependencies — install only the pair for the library you use. **If you are using viem, import from `@openzeppelin/hardhat-upgrades/viem`** (not `@openzeppelin/hardhat-upgrades`) in both your config and your scripts/tests. The examples below use ethers; see [If you are using viem](#if-you-are-using-viem) for the viem equivalents.
</Callout>
