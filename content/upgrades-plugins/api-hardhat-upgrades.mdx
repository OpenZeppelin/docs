...
* **If you are using viem:** the same functions identify contracts by name (a string) and return [viem contract instances](https://viem.sh/docs/contract/getContract); addresses use viem's `Address` type (a `` `0x${string}` `` template-literal type — any string beginning with `0x`).
...
```ts
async function prepareUpgrade(
  referenceAddressOrContract: Address,
  contractName: string,
  opts?:
    unsafeAllow?: ValidationError[],
    unsafeAllowRenames?: boolean,
