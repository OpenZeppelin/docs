...
```mermaid
flowchart LR
    subgraph EVM["EVM Adapter"]
        direction TB
        E1["13/13 capabilities"]
        E2["Ethereum, Polygon,\nArbitrum, Base, ..."]
    end

    subgraph Stellar["Stellar Adapter"]
        direction TB
        S1["13/13 capabilities"]
        S2["Stellar Public,\nStellar Testnet"]
    end

    subgraph Polkadot["Polkadot Adapter"]
        direction TB
        P1["EVM-path capabilities"]
        P2["Polkadot Hub,\nMoonbeam, Moonriver"]
    end

    subgraph Midnight["Midnight Adapter"]
        direction TB
        M1["ZK artifact workflows"]
        M2["Midnight Testnet"]
    end

    subgraph Solana["Solana Adapter"]
        direction TB
        So1["Scaffolding only"]
        So2["Devnet, Testnet"]
    end

    style EVM fill:#e3f2fd,stroke:#1976d2,color:#000
    style Stellar fill:#fff3e0,stroke:#f57c00,color:#000
    style Polkadot fill:#f3e5f5,stroke:#7b1fa2,color:#000
    style Midnight fill:#e0f2f1,stroke:#00897b,color:#000
    style Solana fill:#fafafa,stroke:#9e9e9e,color:#000
```
