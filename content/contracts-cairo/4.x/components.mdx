...
See [ERC20Component](/contracts-cairo/4.x/api/erc20#erc20component) as an example which includes:
...
Let’s look at how a contract would integrate [OwnableComponent](/contracts-cairo/4.x/api/access#OwnableComponent):
...
Here’s an example of how to use the Immutable Component Config pattern with the [ERC2981Component](/contracts-cairo/4.x/api/token_common#erc2981component):
...
For instance, [AccessControlComponent](/contracts-cairo/4.x/api/access#accesscontrolcomponent) depends on [SRC5Component](/contracts-cairo/4.x/api/introspection#src5component).
...
Next, the contract includes the [ERC20Component](/contracts-cairo/4.x/api/erc20#erc20component) implementations; however, `ERC20Impl` and `ERC20CamelOnlyImpl` are **not** embedded.
