...
The ERC20 token standard is a specification for [fungible tokens](https://docs.openzeppelin.com/contracts/5.x/tokens#different-kinds-of-tokens), a type of token where all the units are exactly equal to each other.
...
`MyToken` integrates the `ERC20MixinImpl` with the embed directive which marks its implementations as external in the contract.
The mixin bundles the ERC20 and ERC20 Metadata implementations, along with the deprecated `camelCase` entrypoints.
Including the metadata methods is generally recommended because the vast majority of ERC20 tokens provide them.
...
The following interface represents the full ABI of the Contracts for Cairo [ERC20Component](/contracts-cairo/4.x/api/erc20#ERC20Component).
The interface includes the [IERC20](/contracts-cairo/4.x/api/erc20#IERC20) standard interface as well as the optional [IERC20Metadata](/contracts-cairo/4.x/api/erc20#IERC20Metadata).
...
The [SafeERC20DispatcherTrait](/contracts-cairo/4.x/api/erc20#SafeERC20DispatcherTrait) wraps calls made through an `IERC20Dispatcher` and panics when the token returns `false`. It provides `assert_transfer`, `assert_transfer_from`, `assert_increase_allowance`, and `assert_decrease_allowance`:
...
The helpers provide a consistent failure path for conforming ERC20 contracts. They do not add compatibility with contracts that return data outside the declared Cairo ABI; strict dispatcher deserialization already rejects those responses.

### Flash minting

The [ERC20FlashMintComponent](/contracts-cairo/4.x/api/erc20#ERC20FlashMintComponent) implements the ERC-3156 flash-lender interface by minting tokens for the duration of a loan and recovering them, plus any configured fee, before the transaction completes. Implement `FlashMintConfigTrait` to customize the maximum loan, fee, or fee recipient. The default configuration charges no fee; if a custom configuration charges a nonzero fee while `flash_fee_receiver` returns the zero address, that fee is burned.

<Callout type='warn'>
The default `max_flash_loan` is derived from the numeric limit and the current total supply. It does not account for an additional supply cap imposed by `ERC20Votes`. Avoid combining these extensions, or override `FlashMintConfigTrait` so the reported maximum respects that cap.
</Callout>

### Wrapping an underlying token

The [ERC20WrapperComponent](/contracts-cairo/4.x/api/erc20#ERC20WrapperComponent) lets users deposit an underlying ERC20 and mint the same amount of wrapped tokens, or burn wrapped tokens to withdraw the underlying asset. The initializer requires the underlying token to implement `IERC20Metadata` and use the same number of decimals as the wrapper.

<Callout type='warn'>
An underlying token that changes balances without explicit transfers, such as a rebasing token, can leave the wrapper undercollateralized. Restrict any exposed recovery function appropriately and verify the underlying token’s accounting model before integrating it.
</Callout>
