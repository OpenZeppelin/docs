...
OpenZeppelin Contracts for Cairo provides [OwnableComponent](/contracts-cairo/4.x/api/access#OwnableComponent) for implementing ownership in your contracts.
...
[`initializer`](/contracts-cairo/4.x/api/access#OwnableComponent-initializer) like this:
...
[OwnableTwoStepImpl](/contracts-cairo/4.x/api/access#OwnableComponent-Embeddable-Impls-OwnableTwoStepImpl) implementation. A two step transfer mechanism helps
...
instead of simply using [`assert_only_owner`](/contracts-cairo/4.x/api/access#OwnableComponent-assert_only_owner). This check can be enforced through [`assert_only_role`](/contracts-cairo/4.x/api/access#AccessControlComponent-assert_only_role).
...
Here’s a simple example of implementing [AccessControl](/contracts-cairo/4.x/api/access#AccessControlComponent) on a portion of an ERC20 token contract which defines
...
<Callout type='warn'>
Make sure you fully understand how [AccessControl](/contracts-cairo/4.x/api/access#AccessControlComponent) works before
...
[Ownable](/contracts-cairo/4.x/api/access#OwnableComponent). Where [AccessControl](/contracts-cairo/4.x/api/access#AccessControlComponent) shines the most is in scenarios where granular
...
The ERC20 token example above uses [`_grant_role`](/contracts-cairo/4.x/api/access#AccessControlComponent-_grant_role),
...
the [`assert_only_role`](/contracts-cairo/4.x/api/access#AccessControlComponent-assert_only_role) check pass. To grant and revoke roles dynamically, you will need help from the role’s _admin_.

Every role has an associated admin role, which grants permission to call the
[`grant_role`](/contracts-cairo/4.x/api/access#AccessControlComponent-grant_role) and
[`revoke_role`](/contracts-cairo/4.x/api/access#AccessControlComponent-revoke_role) functions.
...
[`set_role_admin`](/contracts-cairo/4.x/api/access#AccessControlComponent-set_role_admin) is used to select a new admin role.

Since it is the admin for all roles by default, and in fact it is also its own admin, this role carries significant risk. To mitigate this risk we provide [AccessControlDefaultAdminRules](/contracts-cairo/4.x/api/access#AccessControlDefaultAdminRulesComponent), a recommended extension of AccessControl that adds a number of enforced security measures for this role: the admin is restricted to a single account, with a 2-step transfer procedure with a delay in between steps.
