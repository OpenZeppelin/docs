...
<Callout type='warn'>
`ERC4626SelfAssetsManagement` uses `SafeERC20DispatcherTrait` for transfers. An ERC20 operation that returns `false` fails with `'SafeERC20: failed operation'`. See [Migrating from 3.x](/contracts-cairo/4.x/backwards-compatibility#migrating-from-3x) when updating error imports or assertions.
</Callout>
...
compliant with the ERC4626 requirements regarding the preview functions. Fees are calculated through the [FeeConfigTrait](/contracts-cairo/4.x/api/erc20#ERC4626Component-FeeConfigTrait)
implementation. By default, the ERC4626 component charges no fees. If this is the desired behavior, you can use the default
[ERC4626DefaultNoFees](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/token/src/erc20/extensions/erc4626/erc4626.cairo) implementation.

<Callout>
Fees can be charged in either assets or shares. See the [FeeConfigTrait](/contracts-cairo/4.x/api/erc20#ERC4626Component-FeeConfigTrait) and implementation examples in [ERC4626 mocks](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/test_common/src/mocks/erc4626.cairo).
</Callout>
            ...
            if let Option::Some(fee) = fee {
                ...
                match fee {
                    Fee::Assets(fee) => {
                        ...
                        asset_dispatcher.assert_transfer(fee_recipient, fee);
                    },
...
The following interface represents the full ABI of the Contracts for Cairo [ERC4626Component](/contracts-cairo/4.x/api/erc20#ERC4626Component).
The full interface includes the [IERC4626](/contracts-cairo/4.x/api/erc20#IERC4626), [IERC20](/contracts-cairo/4.x/api/erc20#IERC20), and [IERC20Metadata](/contracts-cairo/4.x/api/erc20#IERC20Metadata) interfaces.
