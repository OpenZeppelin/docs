---
title: SNIP12 and Typed Messages
---
...
using these utilities for on-chain signature verification. For that, let’s build an example with a custom [ERC20](/contracts-cairo/4.x/api/erc20#ERC20Component) contract
...
Let’s start with a basic ERC20 contract leveraging the [ERC20Component](/contracts-cairo/4.x/api/erc20#ERC20Component), and let’s add the new function.
...
<Callout>
We are using the [`ISRC6Dispatcher`](/contracts-cairo/4.x/api/account#ISRC6) to verify the signature,
and the [`NoncesComponent`](/contracts-cairo/4.x/api/utilities#NoncesComponent) to handle nonces to prevent replay attacks.
</Callout>
