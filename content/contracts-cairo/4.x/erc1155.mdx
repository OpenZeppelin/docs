...
is why its [balance_of](/contracts-cairo/4.x/api/erc1155#IERC1155-balance_of) function differs from ERC20’s and ERC777’s: it has an additional ID argument for the
...
non-fungible and exists or doesn’t. The ERC721 [balance_of](/contracts-cairo/4.x/api/erc721#IERC721-balance_of) function refers to how many different tokens an account
...
The following interface represents the full ABI of the Contracts for Cairo [ERC1155Component](/contracts-cairo/4.x/api/erc1155#ERC1155Component).
The interface includes the [IERC1155](/contracts-cairo/4.x/api/erc1155#IERC1155) standard interface and the optional [IERC1155MetadataURI](/contracts-cairo/4.x/api/erc1155#IERC1155MetadataURI) interface together with [ISRC5](/contracts-cairo/4.x/api/introspection#ISRC5).
...
Because all state is held in a single contract, it is possible to operate over multiple tokens in a single transaction very efficiently. The standard provides two functions, [balance_of_batch](/contracts-cairo/4.x/api/erc1155#IERC1155-balance_of_batch) and [safe_batch_transfer_from](/contracts-cairo/4.x/api/erc1155#IERC1155-safe_batch_transfer_from), that make querying multiple balances and transferring multiple tokens simpler and less gas-intensive. We also have [safe_transfer_from](/contracts-cairo/4.x/api/erc1155#IERC1155-safe_transfer_from) for non-batch operations.

In the spirit of the standard, we’ve also included batch operations in the non-standard functions, such as
[batch_mint_with_acceptance_check](/contracts-cairo/4.x/api/erc1155#ERC1155Component-batch_mint_with_acceptance_check).

<Callout type='warn'>
While [safe_transfer_from](/contracts-cairo/4.x/api/erc1155#IERC1155-safe_transfer_from) and [safe_batch_transfer_from](/contracts-cairo/4.x/api/erc1155#IERC1155-safe_batch_transfer_from) prevent loss by checking the receiver can handle the
...
The [ERC1155URIStorageComponent](/contracts-cairo/4.x/api/erc1155#ERC1155URIStorageComponent) supports a base URI plus an individual URI suffix for each token ID. Use `ERC1155URIStorageImpl` instead of `ERC1155TokenURIDefaultImpl`; IDs without a stored suffix fall back to the URI held by the core component. Calling `set_token_uri` emits the standard `URI` event with the resolved value.
...
Implementing the `IERC1155Receiver` interface exposes the [on_erc1155_received](/contracts-cairo/4.x/api/erc1155#IERC1155Receiver-on_erc1155_received) and [on_erc1155_batch_received](/contracts-cairo/4.x/api/erc1155#IERC1155Receiver-on_erc1155_batch_received) methods.
When [safe_transfer_from](/contracts-cairo/4.x/api/erc1155#IERC1155-safe_transfer_from) and [safe_batch_transfer_from](/contracts-cairo/4.x/api/erc1155#IERC1155-safe_batch_transfer_from) are called, they invoke the recipient contract’s `on_erc1155_received` or `on_erc1155_batch_received` methods respectively which **must** return the [IERC1155Receiver interface ID](/contracts-cairo/4.x/api/erc1155#IERC1155Receiver).
