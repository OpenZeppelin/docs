...
[reusable components](/contracts-cairo/4.x/components) to build custom smart contracts, as well as ready-to-deploy [presets](/contracts-cairo/4.x/presets). You can also
find other [utilities](/contracts-cairo/4.x/api/utilities) including [interfaces and dispatchers](/contracts-cairo/4.x/interfaces) and [test utilities](/contracts-cairo/4.x/api/testing)
that facilitate testing with Starknet Foundry.

Version 4.x provides [ERC-6909 multi-token contracts](/contracts-cairo/4.x/erc6909), ERC-3156 flash minting, ERC20 and ERC721 wrappers, consecutive ERC721 minting, ERC1155 supply tracking, per-token URI storage, and safe ERC20 dispatcher helpers. See [Migrating from 3.x](/contracts-cairo/4.x/backwards-compatibility#migrating-from-3x) before upgrading an existing project.
...
The `openzeppelin` package is an umbrella (meta) package that aggregates all library subpackages. The umbrella package,
`openzeppelin_interfaces`, `openzeppelin_utils`, and `openzeppelin_testing` use independent version numbers.

See the [Versioning of the sub-packages](/contracts-cairo/4.x#versioning-of-the-sub-packages) section for more information.
...
This is how it looks to build an ERC20 contract using the [ERC20 component](/contracts-cairo/4.x/erc20).
