...
The macro recognizes the following token extension component names:

* `ERC20FlashMint` and `ERC20Wrapper`
* `ERC721Consecutive`, `ERC721URIStorage`, and `ERC721Wrapper`
* `ERC1155Supply` and `ERC1155URIStorage`
* `ERC6909`, `ERC6909ContentURI`, `ERC6909Metadata`, and `ERC6909TokenSupply`

For example, an ERC20 with flash lending can declare `#[with_components(ERC20, ERC20FlashMint)]`, while an ERC6909 contract with metadata and supply tracking can declare `#[with_components(ERC6909, ERC6909Metadata, ERC6909TokenSupply)]`.

The diagnostics recognize fully qualified trait paths and `initializer_no_metadata`. They report incompatible ERC721 enumerable/consecutive integrations, missing hook forwarding for token supply, URI storage, enumeration, consecutive ownership, or voting checkpoints, and combinations such as flash minting or consecutive minting with Votes that require custom supply accounting.

<Callout type='warn'>
`with_components` can detect many missing hook calls, but it does not wire extension hooks automatically. Follow each component’s integration requirements; ignoring a warning can leave supply, ownership, URI, or voting data out of sync.
</Callout>
