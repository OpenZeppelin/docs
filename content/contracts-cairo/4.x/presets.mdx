...
<Callout>
Class hashes were computed with Scarb `v{{class_hash_scarb_version}}` and the `scarb --release` profile.
</Callout>
...
| --- | --- |
| [`AccountUpgradeable`](/contracts-cairo/4.x/api/account#AccountUpgradeable) | `{{AccountUpgradeableClassHash}}` |
| [`ERC20Upgradeable`](/contracts-cairo/4.x/api/erc20#ERC20Upgradeable) | `{{ERC20UpgradeableClassHash}}` |
...
| [`UniversalDeployer`](/contracts-cairo/4.x/api/udc#UniversalDeployer) | `{{UniversalDeployerClassHash}}` |
| [`VestingWallet`](/contracts-cairo/4.x/api/finance#VestingWallet) | `{{VestingWalletClassHash}}` |
...
```bash
starkli deploy {{ERC20UpgradeableClassHash}} \
  <CONSTRUCTOR_ARGS> \
  --network="sepolia"
```

If a class hash has yet to be declared, copy/paste the preset contract code and declare it locally.
Start by [setting up a project](/contracts-cairo/4.x#set-up-your-project) and [installing the Contracts for Cairo library](/contracts-cairo/4.x#install-the-library).
Copy the target preset contract from the [presets directory](https://github.com/OpenZeppelin/cairo-contracts/tree/{{api_source_version}}/packages/presets/src)
