...
The [VestingComponent](/contracts-cairo/4.x/api/finance#VestingComponent) manages the gradual release of ERC-20 tokens to a designated beneficiary based on a predefined vesting schedule.
The implementing contract must implement the [OwnableComponent](/contracts-cairo/4.x/api/access#OwnableComponent), where the contract owner is regarded as the vesting beneficiary.
...
<Callout type='warn'>
`release` uses `SafeERC20DispatcherTrait` to transfer vested tokens. An ERC20 operation that returns `false` fails with `'SafeERC20: failed operation'`. See [Migrating from 3.x](/contracts-cairo/4.x/backwards-compatibility#migrating-from-3x) when updating error imports or assertions.
</Callout>

### Vesting schedule

The [VestingSchedule](/contracts-cairo/4.x/api/finance#VestingComponent-Vesting-Schedule-functions) trait defines the logic for calculating the vested amount based on a given timestamp. This
logic is not part of the [VestingComponent](/contracts-cairo/4.x/api/finance#VestingComponent), so any contract implementing the [VestingComponent](/contracts-cairo/4.x/api/finance#VestingComponent) must provide its own
implementation of the [VestingSchedule](/contracts-cairo/4.x/api/finance#VestingComponent-Vesting-Schedule-functions) trait.

<Callout>
There’s a ready-made implementation of the [VestingSchedule](/contracts-cairo/4.x/api/finance#VestingComponent-Vesting-Schedule-functions) trait available named [LinearVestingSchedule](/contracts-cairo/4.x/api/finance#LinearVestingSchedule).
...
The contract must integrate [VestingComponent](/contracts-cairo/4.x/api/finance#VestingComponent) and [OwnableComponent](/contracts-cairo/4.x/api/access#OwnableComponent) as dependencies. The contract’s constructor
...
The implementing contract must provide an implementation of the [VestingSchedule](/contracts-cairo/4.x/api/finance#VestingComponent-Vesting-Schedule-functions) trait. This can be achieved either by importing
a ready-made [LinearVestingSchedule](/contracts-cairo/4.x/api/finance#LinearVestingSchedule) implementation or by defining a custom one.

Here’s an example of a simple vesting wallet contract with a [LinearVestingSchedule](/contracts-cairo/4.x/api/finance#LinearVestingSchedule), where the vested amount
...
A vesting schedule will often follow a custom formula. In such cases, the [VestingSchedule](/contracts-cairo/4.x/api/finance#VestingComponent-Vesting-Schedule-functions) trait is useful.
To support a custom vesting schedule, the contract must provide an implementation of the
[calculate_vested_amount](/contracts-cairo/4.x/api/finance#VestingComponent-calculate_vested_amount) function based on the desired formula.

<Callout>
When using a custom [VestingSchedule](/contracts-cairo/4.x/api/finance#VestingComponent-Vesting-Schedule-functions) implementation, the [LinearVestingSchedule](/contracts-cairo/4.x/api/finance#LinearVestingSchedule) must be excluded from the imports.
</Callout>
...
Here’s an example of a vesting wallet contract with a custom [VestingSchedule](/contracts-cairo/4.x/api/finance#VestingComponent-Vesting-Schedule-functions) implementation, where tokens
