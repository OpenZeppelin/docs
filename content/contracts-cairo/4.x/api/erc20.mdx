...
<APIGithubLinkHeader
    moduleName="IERC20"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/interfaces/src/token/erc20.cairo`}
/>
...
<APIItem
    ...
    kind="external"
>
Returns the remaining number of tokens that `spender` is allowed to spend on behalf of `owner` through [transfer\_from](#IERC20-transfer_from). This is zero by default.
...
Emits an [Approval](#IERC20-Approval) event.
</APIItem>
...
Tokens usually opt for a value of `18`, imitating the relationship between Ether and Wei. This is the default value returned by this function. To create a custom decimals implementation, see [Customizing decimals](../erc20#customizing-decimals).
...
<APIGithubLinkHeader
    moduleName="IERC4626"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/interfaces/src/token/erc4626.cairo`}
/>
...
### `IERC3156FlashLender` [toc] [#IERC3156FlashLender]

<APIGithubLinkHeader
    moduleName="IERC3156FlashLender"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/interfaces/src/token/erc3156.cairo`}
/>

```rust
use openzeppelin_interfaces::erc3156::IERC3156FlashLender;
```

Interface for an [ERC-3156](https://eips.ethereum.org/EIPS/eip-3156) flash lender.

Functions

- [`max_flash_loan(token)`](#IERC3156FlashLender-max_flash_loan)
- [`flash_fee(token, amount)`](#IERC3156FlashLender-flash_fee)
- [`flash_loan(receiver, token, amount, data)`](#IERC3156FlashLender-flash_loan)

#### Functions [!toc] [#IERC3156FlashLender-Functions]

<APIItem
    functionSignature="max_flash_loan(token: ContractAddress) → u256"
    id="IERC3156FlashLender-max_flash_loan"
    kind="external"
>
Returns the maximum amount of `token` available for a flash loan.
</APIItem>

<APIItem
    functionSignature="flash_fee(token: ContractAddress, amount: u256) → u256"
    id="IERC3156FlashLender-flash_fee"
    kind="external"
>
Returns the fee charged to borrow `amount` of `token`.
</APIItem>

<APIItem
    functionSignature="flash_loan(receiver: ContractAddress, token: ContractAddress, amount: u256, data: Span<felt252>) → bool"
    id="IERC3156FlashLender-flash_loan"
    kind="external"
>
Initiates a flash loan and returns `true` when the loan completes successfully. The lender passes `data` to the receiver callback.
</APIItem>

### `IERC3156FlashBorrower` [toc] [#IERC3156FlashBorrower]

<APIGithubLinkHeader
    moduleName="IERC3156FlashBorrower"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/interfaces/src/token/erc3156.cairo`}
/>
...
Interface for contracts that receive [ERC-3156](https://eips.ethereum.org/EIPS/eip-3156) flash loans.

Functions

- [`on_flash_loan(initiator, token, amount, fee, data)`](#IERC3156FlashBorrower-on_flash_loan)

#### Functions [!toc] [#IERC3156FlashBorrower-Functions]

<APIItem
    functionSignature="on_flash_loan(initiator: ContractAddress, token: ContractAddress, amount: u256, fee: u256, data: Span<felt252>) → felt252"
    id="IERC3156FlashBorrower-on_flash_loan"
    kind="external"
>
Handles a flash loan callback. A successful callback returns `selector!("ERC3156FlashBorrower.onFlashLoan")`.
</APIItem>

### `IERC20Wrapper` [toc] [#IERC20Wrapper]

<APIGithubLinkHeader
    moduleName="IERC20Wrapper"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/interfaces/src/token/erc20.cairo`}
/>

```rust
use openzeppelin_interfaces::erc20::{IERC20Wrapper, IERC20WrapperABI};
```

Interface for an ERC20 token backed one-for-one by another ERC20 token. [`IERC20WrapperABI`](#IERC20WrapperABI) includes the same functions and also exposes [`recover`](#IERC20Wrapper-recover).

Functions

- [`underlying()`](#IERC20Wrapper-underlying)
- [`deposit_for(receiver, amount)`](#IERC20Wrapper-deposit_for)
- [`withdraw_to(receiver, amount)`](#IERC20Wrapper-withdraw_to)
- [`recover(account)`](#IERC20Wrapper-recover) (`IERC20WrapperABI` only)

#### Functions [!toc] [#IERC20Wrapper-Functions]

<APIItem
    functionSignature="IERC20WrapperABI trait"
    id="IERC20WrapperABI"
    kind="trait"
>
Aggregate wrapper ABI containing `underlying`, `deposit_for`, `withdraw_to`, and `recover`.
</APIItem>

<APIItem
    functionSignature="underlying() → ContractAddress"
    id="IERC20Wrapper-underlying"
    kind="external"
>
...
<APIItem
    functionSignature="deposit_for(receiver: ContractAddress, amount: u256) → bool"
    id="IERC20Wrapper-deposit_for"
    kind="external"
>
Deposits `amount` underlying tokens and mints the same amount of wrapped tokens to `receiver`.
</APIItem>

<APIItem
    functionSignature="withdraw_to(receiver: ContractAddress, amount: u256) → bool"
    id="IERC20Wrapper-withdraw_to"
    kind="external"
>
Burns `amount` wrapped tokens from the caller and transfers the same amount of underlying tokens to `receiver`.
</APIItem>

<APIItem
    functionSignature="recover(account: ContractAddress) → u256"
    id="IERC20Wrapper-recover"
    kind="external"
>
Mints wrapped tokens to `account` for recoverable excess underlying tokens and returns the amount minted. This function belongs to `IERC20WrapperABI`, not `IERC20Wrapper`.
</APIItem>
...
<APIGithubLinkHeader
    ...
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/token/src/erc20/erc20.cairo`}
/>
...
<APIItem
    functionSignature="transferFrom(ref self: ContractState, sender: ContractAddress, recipient: ContractAddress, amount: u256) → bool"
...
Emits an [Approval](#ERC20Component-Approval) event. Every successful call increases `owner`'s nonce by one.
</APIItem>
...
<APIGithubLinkHeader
    ...
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/token/src/erc20/extensions/erc4626/erc4626.cairo`}
/>
...
#### FeeConfigTrait [!toc] [#ERC4626Component-FeeConfigTrait-Overview]
...
#### LimitConfigTrait [!toc] [#ERC4626Component-LimitConfigTrait-Overview]
...
#### ERC4626HooksTrait [!toc] [#ERC4626Component-ERC4626HooksTrait-Overview]
...
#### AssetsManagementTrait [!toc] [#ERC4626Component-AssetsManagementTrait-Overview]
...
- Contract charging fees in assets: [ERC4626AssetsFeesMock](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/test_common/src/mocks/erc4626.cairo#L253)
- Contract charging fees in shares: [ERC4626SharesFeesMock](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/test_common/src/mocks/erc4626.cairo#L426)
...
See the [ERC4626AssetsFeesMock](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/test_common/src/mocks/erc4626.cairo#L253) and [ERC4626SharesFeesMock](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/test_common/src/mocks/erc4626.cairo#L426) examples.
...
- Self-managed vault: [ERC4626SelfAssetsManagement](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/token/src/erc20/extensions/erc4626/erc4626.cairo#L760).
- External vault: [ERC4626ExternalAssetsManagement](https://github.com/OpenZeppelin/cairo-contracts/blob/{{api_source_version}}/packages/test_common/src/mocks/erc4626.cairo#L92).
...
The underlying transfer is executed through [`SafeERC20DispatcherTrait::assert_transfer_from`](#SafeERC20DispatcherTrait-assert_transfer_from), which panics if the token returns `false`.
...
The underlying transfer is executed through [`SafeERC20DispatcherTrait::assert_transfer`](#SafeERC20DispatcherTrait-assert_transfer), which panics if the token returns `false`.
...
### `ERC20FlashMintComponent` [toc] [#ERC20FlashMintComponent]

<APIGithubLinkHeader
    moduleName="ERC20FlashMintComponent"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/token/src/erc20/extensions/erc20_flash_mint.cairo`}
/>

```rust
use openzeppelin_token::erc20::extensions::erc20_flash_mint::{DefaultConfig, ERC20FlashMintComponent};
```

Component that adds [ERC-3156](https://eips.ethereum.org/EIPS/eip-3156) flash loans to an [`ERC20Component`](#ERC20Component) by minting the borrowed amount for the duration of the loan.

Configuration

- [`FlashMintConfigTrait`](#ERC20FlashMintComponent-FlashMintConfigTrait)
- [`DefaultConfig`](#ERC20FlashMintComponent-DefaultConfig)

Embeddable Implementations

- [`ERC20FlashMintImpl`](#ERC20FlashMintComponent-ERC20FlashMintImpl)

Errors

- [`UNSUPPORTED_TOKEN`](#ERC20FlashMintComponent-UNSUPPORTED_TOKEN)
- [`EXCEEDED_MAX_LOAN`](#ERC20FlashMintComponent-EXCEEDED_MAX_LOAN)
- [`INVALID_RECEIVER`](#ERC20FlashMintComponent-INVALID_RECEIVER)

#### `FlashMintConfigTrait` [!toc] [#ERC20FlashMintComponent-FlashMintConfigTrait]

Override this trait to customize the maximum loan, fee calculation, or fee recipient.

<APIItem
    functionSignature="max_flash_loan(self: @ComponentState, token: ContractAddress, total_supply: u256) → u256"
    id="ERC20FlashMintComponent-Config-max_flash_loan"
    kind="internal"
>
Returns the maximum amount available for a flash loan. The default implementation returns zero for any token other than the current contract and otherwise returns `u256::MAX - total_supply`.

Contracts with another supply cap must override this function so the reported maximum respects that cap. The default is therefore unsuitable when this component is combined with `ERC20VotesComponent` unless the configuration accounts for its supply limit.
</APIItem>

<APIItem
    functionSignature="flash_fee(self: @ComponentState, token: ContractAddress, amount: u256) → u256"
    id="ERC20FlashMintComponent-Config-flash_fee"
    kind="internal"
>
Returns the fee for borrowing `amount` of `token`. The default implementation returns zero.
</APIItem>

<APIItem
    functionSignature="flash_fee_receiver(self: @ComponentState) → ContractAddress"
    id="ERC20FlashMintComponent-Config-flash_fee_receiver"
    kind="internal"
>
Returns the fee recipient. The default implementation returns the zero address, causing the fee to be burned.
</APIItem>

<APIItem
    functionSignature="DefaultConfig impl"
    id="ERC20FlashMintComponent-DefaultConfig"
    kind="impl"
>
Default implementation of [`FlashMintConfigTrait`](#ERC20FlashMintComponent-FlashMintConfigTrait).
</APIItem>

#### `ERC20FlashMintImpl` [!toc] [#ERC20FlashMintComponent-ERC20FlashMintImpl]

- [`max_flash_loan(self, token)`](#ERC20FlashMintComponent-max_flash_loan)
- [`flash_fee(self, token, amount)`](#ERC20FlashMintComponent-flash_fee)
- [`flash_loan(self, receiver, token, amount, data)`](#ERC20FlashMintComponent-flash_loan)

<APIItem
    functionSignature="max_flash_loan(self: @ContractState, token: ContractAddress) → u256"
    id="ERC20FlashMintComponent-max_flash_loan"
    kind="external"
>
Returns the maximum amount of `token` available for a flash loan, as determined by [`FlashMintConfigTrait`](#ERC20FlashMintComponent-FlashMintConfigTrait).
</APIItem>

<APIItem
    functionSignature="flash_fee(self: @ContractState, token: ContractAddress, amount: u256) → u256"
    id="ERC20FlashMintComponent-flash_fee"
    kind="external"
>
Returns the configured fee for borrowing `amount`. Panics with [`UNSUPPORTED_TOKEN`](#ERC20FlashMintComponent-UNSUPPORTED_TOKEN) when `token` is not the current contract.
</APIItem>

<APIItem
    functionSignature="flash_loan(ref self: ContractState, receiver: ContractAddress, token: ContractAddress, amount: u256, data: Span<felt252>) → bool"
    id="ERC20FlashMintComponent-flash_loan"
    kind="external"
>
Mints `amount` tokens to `receiver`, calls `IERC3156FlashBorrower::on_flash_loan`, and requires the receiver to approve the component for `amount + fee` before the call returns. It then burns the principal and either burns the fee or transfers it to the configured fee recipient.

The receiver callback must return `selector!("ERC3156FlashBorrower.onFlashLoan")`. Any callback reentrancy is safe with respect to the loan because the minted tokens are recovered before completion or the whole transaction reverts.
</APIItem>

#### Errors [!toc] [#ERC20FlashMintComponent-Errors]

<APIItem
    functionSignature="UNSUPPORTED_TOKEN: felt252"
    id="ERC20FlashMintComponent-UNSUPPORTED_TOKEN"
    kind="constant"
>
`'FlashMint: unsupported token'`
</APIItem>

<APIItem
    functionSignature="EXCEEDED_MAX_LOAN: felt252"
    id="ERC20FlashMintComponent-EXCEEDED_MAX_LOAN"
    kind="constant"
>
`'FlashMint: exceeded max loan'`
</APIItem>

<APIItem
    functionSignature="INVALID_RECEIVER: felt252"
    id="ERC20FlashMintComponent-INVALID_RECEIVER"
    kind="constant"
>
`'FlashMint: invalid receiver'`
</APIItem>

### `ERC20WrapperComponent` [toc] [#ERC20WrapperComponent]

<APIGithubLinkHeader
    moduleName="ERC20WrapperComponent"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/token/src/erc20/extensions/erc20_wrapper.cairo`}
/>
...
Component for an ERC20 token backed one-for-one by an underlying ERC20 token. The underlying token must implement `IERC20Metadata` and use the same decimals as the wrapper.

Tokens whose balances can change without explicit transfers, such as rebasing tokens, can desynchronize the wrapped supply from the underlying balance. [`recover`](#ERC20WrapperComponent-recover) can mint wrapped tokens for a positive excess, but cannot repair undercollateralization.

Embeddable Implementations

- [`ERC20WrapperImpl`](#ERC20WrapperComponent-ERC20WrapperImpl)

Internal Implementations

- [`InternalImpl`](#ERC20WrapperComponent-InternalImpl)

Events

- [`Deposit(sender, receiver, assets)`](#ERC20WrapperComponent-Deposit)
- [`Withdraw(caller, receiver, assets)`](#ERC20WrapperComponent-Withdraw)

#### Storage [!toc] [#ERC20WrapperComponent-Storage]

<APIItem
    functionSignature="ERC20Wrapper_underlying: ContractAddress"
    id="ERC20WrapperComponent-ERC20Wrapper_underlying"
    kind="storage"
>
The configured underlying token address.
</APIItem>

#### `ERC20WrapperImpl` [!toc] [#ERC20WrapperComponent-ERC20WrapperImpl]

- [`underlying(self)`](#ERC20WrapperComponent-underlying)
- [`deposit_for(self, receiver, amount)`](#ERC20WrapperComponent-deposit_for)
- [`withdraw_to(self, receiver, amount)`](#ERC20WrapperComponent-withdraw_to)

<APIItem
    functionSignature="underlying(self: @ContractState) → ContractAddress"
    id="ERC20WrapperComponent-underlying"
    kind="external"
>
Returns the address of the underlying ERC20 token.
</APIItem>
...
<APIItem
    functionSignature="deposit_for(ref self: ContractState, receiver: ContractAddress, amount: u256) → bool"
    id="ERC20WrapperComponent-deposit_for"
    kind="external"
>
Transfers `amount` underlying tokens from the caller to the wrapper, mints the same amount of wrapped tokens to `receiver`, emits [`Deposit`](#ERC20WrapperComponent-Deposit), and returns `true`.

The caller cannot be the wrapper itself. The receiver cannot be the zero address or the wrapper.
</APIItem>

<APIItem
    functionSignature="withdraw_to(ref self: ContractState, receiver: ContractAddress, amount: u256) → bool"
    id="ERC20WrapperComponent-withdraw_to"
    kind="external"
>
Burns `amount` wrapped tokens from the caller, transfers the same amount of underlying tokens to `receiver`, emits [`Withdraw`](#ERC20WrapperComponent-Withdraw), and returns `true`.

The receiver cannot be the zero address or the wrapper.
</APIItem>

#### `InternalImpl` [!toc] [#ERC20WrapperComponent-InternalImpl]

- [`initializer(self, underlying)`](#ERC20WrapperComponent-initializer)
- [`recover(self, account)`](#ERC20WrapperComponent-recover)

<APIItem
    functionSignature="initializer(ref self: ContractState, underlying: ContractAddress)"
    id="ERC20WrapperComponent-initializer"
    kind="internal"
>
Stores the underlying token address. The address must be non-zero, must differ from the wrapper address, must implement `IERC20Metadata`, and must report the same decimals as the wrapper's `ERC20Component` immutable configuration.
</APIItem>

<APIItem
    functionSignature="recover(ref self: ContractState, account: ContractAddress) → u256"
    id="ERC20WrapperComponent-recover"
    kind="internal"
>
Mints wrapped tokens to `account` equal to the underlying balance held by the wrapper minus the wrapped token's total supply, then returns that amount. Panics when there is no positive excess.

Expose this function with suitable access control when recovery is required.
</APIItem>

#### Events [!toc] [#ERC20WrapperComponent-Events]

<APIItem
    functionSignature="Deposit(sender: ContractAddress, receiver: ContractAddress, assets: u256)"
    id="ERC20WrapperComponent-Deposit"
    kind="event"
>
Emitted when `sender` deposits `assets` underlying tokens and the wrapper mints the corresponding wrapped tokens to `receiver`. `sender` and `receiver` are event keys.
</APIItem>

<APIItem
    functionSignature="Withdraw(caller: ContractAddress, receiver: ContractAddress, assets: u256)"
    id="ERC20WrapperComponent-Withdraw"
    kind="event"
>
Emitted when `caller` burns `assets` wrapped tokens and the wrapper transfers the corresponding underlying tokens to `receiver`. `caller` and `receiver` are event keys.
</APIItem>

#### Errors [!toc] [#ERC20WrapperComponent-Errors]

<APIItem
    functionSignature="INVALID_UNDERLYING_ADDRESS: felt252"
    id="ERC20WrapperComponent-INVALID_UNDERLYING_ADDRESS"
    kind="constant"
>
...
<APIItem
    functionSignature="INVALID_SENDER: felt252"
    id="ERC20WrapperComponent-INVALID_SENDER"
    kind="constant"
>
`'Wrapper: invalid sender'`
</APIItem>

<APIItem
    functionSignature="INVALID_RECEIVER: felt252"
    id="ERC20WrapperComponent-INVALID_RECEIVER"
    kind="constant"
>
`'Wrapper: invalid receiver'`
</APIItem>

<APIItem
    functionSignature="NOTHING_TO_RECOVER: felt252"
    id="ERC20WrapperComponent-NOTHING_TO_RECOVER"
    kind="constant"
>
`'Wrapper: nothing to recover'`
</APIItem>

<APIItem
    functionSignature="INVALID_DECIMALS: felt252"
    id="ERC20WrapperComponent-INVALID_DECIMALS"
    kind="constant"
>
`'Wrapper: invalid decimals'`
</APIItem>

## Utilities [#erc20-utilities]

### `SafeERC20DispatcherTrait` [toc] [#SafeERC20DispatcherTrait]

<APIGithubLinkHeader
    moduleName="SafeERC20DispatcherTrait"
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/token/src/erc20/utils/safe_erc20.cairo`}
/>
...
Extension methods for `IERC20Dispatcher` that panic with consistent errors when an ERC20 operation returns `false`. Cairo's strict ABI deserialization already rejects incompatible return shapes; these helpers do not add support for non-conforming tokens that omit return values.

The `SafeERC20DispatcherImpl` implementation is selected automatically when this trait is in scope.

<APIItem
    functionSignature="SafeERC20DispatcherImpl impl"
    id="SafeERC20DispatcherImpl"
    kind="impl"
>
Implementation of [`SafeERC20DispatcherTrait`](#SafeERC20DispatcherTrait) for `IERC20Dispatcher`.
</APIItem>

Functions

- [`assert_transfer(recipient, amount)`](#SafeERC20DispatcherTrait-assert_transfer)
- [`assert_transfer_from(sender, recipient, amount)`](#SafeERC20DispatcherTrait-assert_transfer_from)
- [`assert_increase_allowance(spender, added_value)`](#SafeERC20DispatcherTrait-assert_increase_allowance)
- [`assert_decrease_allowance(spender, subtracted_value)`](#SafeERC20DispatcherTrait-assert_decrease_allowance)

#### Functions [!toc] [#SafeERC20DispatcherTrait-Functions]

<APIItem
    functionSignature="assert_transfer(self: IERC20Dispatcher, recipient: ContractAddress, amount: u256)"
    id="SafeERC20DispatcherTrait-assert_transfer"
    kind="function"
>
Calls `transfer(recipient, amount)` and panics with `SAFE_ERC20_FAILED_OPERATION` if it returns `false`.
</APIItem>

<APIItem
    functionSignature="assert_transfer_from(self: IERC20Dispatcher, sender: ContractAddress, recipient: ContractAddress, amount: u256)"
    id="SafeERC20DispatcherTrait-assert_transfer_from"
    kind="function"
>
Calls `transfer_from(sender, recipient, amount)` and panics with `SAFE_ERC20_FAILED_OPERATION` if it returns `false`.
</APIItem>

<APIItem
    functionSignature="assert_increase_allowance(self: IERC20Dispatcher, spender: ContractAddress, added_value: u256)"
    id="SafeERC20DispatcherTrait-assert_increase_allowance"
    kind="function"
>
Reads the caller contract's current allowance for `spender`, approves `current + added_value`, and panics with `SAFE_ERC20_FAILED_OPERATION` if `approve` returns `false`.
</APIItem>

<APIItem
    functionSignature="assert_decrease_allowance(self: IERC20Dispatcher, spender: ContractAddress, subtracted_value: u256)"
    id="SafeERC20DispatcherTrait-assert_decrease_allowance"
    kind="function"
>
Reads the caller contract's current allowance for `spender`, requires it to be at least `subtracted_value`, and approves the difference. An insufficient allowance panics with `SAFE_ERC20_FAILED_DECREASE`; a `false` approval result panics with `SAFE_ERC20_FAILED_OPERATION`.
</APIItem>

#### Errors [!toc] [#SafeERC20DispatcherTrait-Errors]

<APIItem
    functionSignature="SAFE_ERC20_FAILED_OPERATION: felt252"
    id="SafeERC20DispatcherTrait-SAFE_ERC20_FAILED_OPERATION"
    kind="constant"
>
`'SafeERC20: failed operation'`
</APIItem>

<APIItem
    functionSignature="SAFE_ERC20_FAILED_DECREASE: felt252"
    id="SafeERC20DispatcherTrait-SAFE_ERC20_FAILED_DECREASE"
    kind="constant"
>
`'SafeERC20: failed decrease'`
</APIItem>
...
<APIGithubLinkHeader
    ...
    link={`https://github.com/OpenZeppelin/cairo-contracts/blob/${API_SOURCE_VERSION}/packages/presets/src/erc20.cairo`}
/>
...
- [`ERC20MixinImpl`](#ERC20Component-Embeddable-Impls-ERC20MixinImpl)

#### OwnableMixinImpl [!toc] [#ERC20Upgradeable-Embedded-Impls-OwnableMixinImpl]

- [`OwnableMixinImpl`](./access#OwnableComponent-Embeddable-Impls-OwnableMixinImpl)
