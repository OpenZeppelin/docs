...
pub trait Policy {
    ...
    /// Validates and enforces the policy's authorization logic.
    /// Must panic if conditions are not met.
...
Enforcement is called during authorization when a context rule is being evaluated. The smart account calls `enforce()` on each policy attached to the rule. Each `enforce()` call both validates that the policy conditions are met and performs any necessary state changes.

If any policy's `enforce()` call panics, the entire authorization fails.

Enforcement allows policies to:
- Validate conditions (e.g., spending limits, signer thresholds, time restrictions)
...
4. **Handle Errors Clearly**: Panic with descriptive error messages in `enforce` when conditions are not met
