...
1. Replace `"signature": "void"` with the proper `AuthPayload` structure (containing `signers` map and `context_rule_ids`)
...
async function signAndSendTx(
    ...
    // 1) Construct the 1st auth entry: `AuthPayload` with `Signer::Delegated(Address)`
...
Removes a signer by its ID from an existing context rule. The rule must maintain at least one signer OR one policy after removal.
