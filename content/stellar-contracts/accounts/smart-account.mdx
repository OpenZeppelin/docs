...
### Policies
Policies act as enforcement modules attached to context rules. They validate and enforce constraints during authorization, panicking if conditions are not met.
...
Authorization is determined by explicitly selecting context rules via `AuthPayload::context_rule_ids`:

1. **Rule Selection**: The caller supplies one rule ID per auth context in the `AuthPayload`
2. **Rule Evaluation**: For each (context, rule_id) pair, verify the rule is not expired and context type matches, then authenticate signers
3. **Policy Enforcement**: If policies are present, call `enforce()` on each. If no policies, all rule signers must be authenticated.
