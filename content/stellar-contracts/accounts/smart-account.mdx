...
To achieve this composability, smart accounts implement the `CustomAccountInterface` and define authorization as data and behavior that can evolve over time. The framework takes a context-centric approach, separating three distinct concerns: authentication (signers prove identity), authorization scope (context rules bind signers and policies to specific operations), and enforcement logic (policies apply business constraints like spending limits or thresholds).
...
graph LR
    ...
    style S1 fill:#4A7FB5,color:#fff
    style S2 fill:#4A7FB5,color:#fff
    style S3 fill:#B8860B,color:#fff
    style S4 fill:#3A7D44,color:#fff
    style S5 fill:#3A7D44,color:#fff
```
