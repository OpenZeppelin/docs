...
graph TB
  subgraph "Provider Tree"
    RP["RuntimeProvider<br/>(manages EcosystemRuntime instances)"]
    WSP["WalletStateProvider<br/>(active network, wallet state, hooks)"]
    App["Your Components"]
  end
...
![Wallet connection UI with network selector and connect options](/uikit/wallet-connect.png)
