...
### Draft or Pre-Final Standards

TRON's own TIPs/TRCs, as well as the EIPs/ERCs inherited from upstream OpenZeppelin Contracts where a standard is implemented as-is, can change in incompatible ways before they reach Final status. For this reason, we avoid shipping implementations of a standard before it is Final. Some exceptions are made for standards that have been published for a long time and seem unlikely to change. Implementations for standards that may have breaking changes are published in files named `draft-*.sol` (for example `draft-ITRC7802.sol` or `draft-IERC6093.sol`), regardless of which standards process they belong to, to make that condition explicit. There is no backwards compatibility guarantee for content in files prefixed with `draft`.
...
We recommend using the Tron Upgrades plugins — [`@openzeppelin/hardhat-tron-upgrades` and `@openzeppelin/tronbox-upgrades`](https://github.com/OpenZeppelin/tron-upgrades), or [`@openzeppelin/foundry-upgrades-tron`](https://github.com/OpenZeppelin/openzeppelin-foundry-upgrades-tron) for Foundry — to ensure storage layout safety of upgrades.
