...
Note that **a contract can also be the owner of another one**! This opens the door to using _composability_ to add additional layers of access control complexity to your contracts: a totally custom multisig or DAO contract that _you_ create can hold ownership, the same way [Gnosis Safe](https://safe.global) or an [Aragon DAO](https://aragon.org) does on Ethereum. Prominent Ethereum projects, such as [MakerDAO](https://makerdao.com), use systems similar to this one.

<Callout type='warn'>
Gnosis Safe and Aragon are Ethereum-ecosystem products and are not deployed on TRON: do not point a TRON contract's ownership at either. If what you actually want is several people jointly controlling a single account rather than a smart contract, TRON has this natively: an account's Owner and Active permissions can each require multiple keys and a signing threshold, with no contract needed. If you specifically need the owner to be a *contract* (for on-chain composability, programmable proposals, etc.), you'll need a custom or TRON-native governance contract; the rest of this page, and the [Governor guide](/tron-contracts/governance), give you the building blocks for that.
</Callout>
...
By default, the `TimelockController` administers itself: the admin role is granted to the timelock contract, and optionally to an additional `admin` account passed at construction. The deployer receives no privileges automatically. The admin role grants the right to assign proposers, executors, and other administrators, through the timelock's own governance process.
...
Additionally, the `AccessManager` stores a _guardian_ as an extra protection for each role. This guardian has the ability to cancel operations that have been scheduled by any role member with an execution delay. An operation can also be canceled by the account that scheduled it, by members of `ADMIN_ROLE`, and, for admin-restricted operations on the manager itself (such as scheduled `grantRole` and `revokeRole` calls), by members of that role's admin role. Consider that a role will have its initial admin and guardian default to the `ADMIN_ROLE` (`0`).
