...
- `ERC165` — inherit this contract if you'd like to support interface detection using a lookup table in contract storage. You can register interfaces using `_registerInterface(bytes4)`: check out example usage as part of the TRC-721 implementation.
...
The library is a thin wrapper around the opcode. It handles edge cases gracefully by returning zero for the current block, future blocks, or blocks older than 256, matching the VM's native behavior. Note that TVM does not implement [EIP-2935](https://eips.ethereum.org/EIPS/eip-2935) history storage, so access cannot be extended beyond the 256-block window.
