...
At a high level, signatures are a set of cryptographic algorithms that allow for a _signer_ to prove himself as the owner of a _private key_ used to authorize a piece of information (generally a transaction). Natively, the TVM supports the Elliptic Curve Digital Signature Algorithm ([ECDSA](https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm)) using the secp256k1 curve, however other signature algorithms such as P256 and RSA are supported.

#### secp256k1 Signatures

`ECDSA` provides functions for recovering and managing secp256k1 account ECDSA signatures (TRON's native account signature scheme). These are often generated via [`web3.eth.sign`](https://web3js.readthedocs.io/en/v1.7.3/web3-eth.html#sign), and form a 65-byte array (of type `bytes` in Solidity) arranged the following way: `[[v (1)], [r (32)], [s (32)]]`.

The data signer can be recovered with `ECDSA.recover`, and its address compared to verify the signature. Native TRON wallets (e.g. TronWeb's `signMessageV2` and TronLink) will hash the data to sign and add the prefix `\x19TRON Signed Message:\n`, so when attempting to recover the signer of a TRON signed message hash, you'll want to use `toTronSignedMessageHash`.
...
These signatures are different from regular secp256k1 signatures in that they use a different elliptic curve to perform operations but have similar security guarantees.
...
On TRON, the `verify` function performs the verification entirely in Solidity (it is equivalent to `verifySolidity`). Unlike the upstream OpenZeppelin implementation, it does not attempt the [RIP-7212](https://github.com/ethereum/RIPs/blob/master/RIPS/rip-7212.md) precompile: the native secp256r1 precompile specified for the TVM by [TIP-7951](https://github.com/tronprotocol/tips/blob/master/tip-7951.md) (at address `0x100`) is not assumed to be enabled. This removes any dependency on precompile availability at the cost of higher Energy consumption per verification.
...
- TRC-1271 signatures from smart contract wallets
- TRC-7913 signatures from keys that don't have their own TRON address
...
For standard signature verification that supports both EOAs and TRC-1271 contracts:
...
#### TRC-1271 Contract Signatures

For smart contract wallets that implement TRC-1271, you can explicitly use:
...
#### TRC-7913 Extended Signatures

TRC-7913 extends signature verification to support keys that don't have their own TRON address. This is useful for integrating non-Ethereum cryptographic curves, hardware devices, or other identity systems.
...
- Otherwise: verification is done using an TRC-7913 verifier

#### Batch Verification

For verifying multiple TRC-7913 signatures at once:
...
- `verify` - can prove that some value is part of a [Merkle tree](https://en.wikipedia.org/wiki/Merkle_tree).
...
In Solidity, it's frequently helpful to know whether or not a contract supports an interface you'd like to use. TRC-165 is a standard that enables runtime interface detection. Contracts provide helpers both for implementing TRC-165 in your contracts and querying other contracts:

- `ITRC165` — this is the TRC-165 interface that defines `supportsInterface`. When implementing TRC-165, you'll conform to this interface.
- `TRC165` — inherit this contract to declare support for an interface: override `supportsInterface` to return `true` for the interface ids your contract implements. Check out example usage as part of the TRC-721 implementation.
- `TRC165Checker` — TRC165Checker simplifies the process of checking whether or not a contract supports an interface you care about.
- include with `using TRC165Checker for address;`
- `myAddress.supportsInterface(bytes4)`
- `myAddress.supportsAllInterfaces(bytes4[])`
...
- `EnumerableSet`: A [set](https://en.wikipedia.org/wiki/Set_(abstract_data_type)) with enumeration capabilities.
- `EnumerableMap`: A `mapping` variant with enumeration capabilities.
- `MerkleTree`: An on-chain [Merkle Tree](https://wikipedia.org/wiki/Merkle_Tree) with helper functions.
- `Heap`: A [binary heap](https://en.wikipedia.org/wiki/Binary_heap) to store elements with priority defined by a comparator function.
...
The `TransientSlot` library supports transient storage through user defined value types ([UDVTs](https://docs.soliditylang.org/en/latest/types.html#user-defined-value-types)), which enables the same value types as in Solidity.
...
This is especially useful for building URL-safe tokenURIs for both `TRC-721` or `TRC-1155`. This library provides a clever way to serve URL-safe [Data URI](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs/) compliant strings to serve on-chain data structures.
...
import {Multicall} from "@openzeppelin/tron-contracts/utils/Multicall.sol";
...
`Blockhash` provides access to historical block hashes through the native `BLOCKHASH` opcode, which exposes the hashes of the most recent 256 blocks.

The library is a thin wrapper around the opcode. It handles edge cases gracefully by returning zero for the current block, future blocks, or blocks older than 256, matching the VM's native behavior. Note that the extended [EIP-2935](https://eips.ethereum.org/EIPS/eip-2935) history storage — specified for the TVM by [TIP-2935](https://github.com/tronprotocol/tips/blob/master/tip-2935.md) at the same canonical address — is not yet active on TRON mainnet, so access cannot currently be extended beyond the 256-block window.
...
<Callout>
Only the 256 most recent block hashes are available. Any query for an older block, the current block, or a future block returns `bytes32(0)`. Treat a zero result as "unavailable" rather than a valid hash.
</Callout>
