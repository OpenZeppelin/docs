---
title: 'Hosted Stellar Relayer on AWS: Operator Deployment Guide'
---
...
<Callout type="warn">
Avoid the inline form `--value "$(openssl rand -hex 32)"`: it places the secret directly on the command line, where it is captured in shell history and visible via `ps`/`/proc` to other users on the host. Pass it through a variable (as above) and `unset` it when done.
</Callout>
