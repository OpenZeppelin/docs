...
<Callout>
To understand the inspiration for a multi token standard, see the [multi token standard](/contracts/5.x/erc1155#multi-token-standard) section within the EIP-1155 docs.
</Callout>
...
We’ll use ERC-6909 to track multiple items in a game, each having their own unique attributes. All item types will be minted to the deployer of the contract, which we can later transfer to players. We’ll also use the [`ERC6909Metadata`](/contracts/5.x/api/token/ERC6909#ERC6909Metadata) extension to add decimals to our fungible items (the vanilla ERC-6909 implementation does not have decimals).
...
<Callout>
For an overview of minting mechanisms, check out [Creating ERC-20 Supply](/contracts/5.x/erc20-supply).
</Callout>
