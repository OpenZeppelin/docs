...
OpenZeppelin provides a simple [`Account`](/contracts/latest/api/account#Account) implementation including only the basic logic to handle user operations in compliance with ERC-4337. Developers who want to build their own account can leverage it to bootstrap custom implementations.

User operations are validated using an [`AbstractSigner`](/contracts/latest/api/utils/cryptography#AbstractSigner), which requires implementing the internal [`_rawSignatureValidation`](/contracts/latest/api/utils/cryptography#AbstractSigner-_rawSignatureValidation-bytes32-bytes-) function, of which we offer a set of implementations to cover a wide customization range. This is the lowest-level signature validation layer and is used to wrap other validation methods like the Account’s [`validateUserOp`](/contracts/latest/api/account#Account-validateUserOp-struct-PackedUserOperation-bytes32-uint256-).

## Setting up an account

To set up an account, you can either start configuring it using our Wizard and selecting a predefined validation scheme, or bring your own logic and start by inheriting [`Account`](/contracts/latest/api/account#Account) from scratch. 
...
<Callout>
Accounts don’t support [ERC-721](/contracts/latest/erc721) and [ERC-1155](/contracts/latest/erc1155) tokens natively since these require the receiving address to implement an acceptance check. It is recommended to inherit [ERC721Holder](/contracts/latest/api/token/ERC721#ERC721Holder), [ERC1155Holder](/contracts/latest/api/token/ERC1155#ERC1155Holder) to include these checks in your account.
</Callout>

### Selecting a signer

Since the minimum requirement of [`Account`](/contracts/latest/api/account#Account) is to provide an implementation of [`_rawSignatureValidation`](/contracts/latest/api/utils/cryptography#AbstractSigner-_rawSignatureValidation-bytes32-bytes-), the library includes specializations of the `AbstractSigner` contract that use custom digital signature verification algorithms. Some examples that you can select from include:

* [`SignerECDSA`](/contracts/latest/api/utils/cryptography#SignerECDSA): Verifies signatures produced by regular EVM Externally Owned Accounts (EOAs).
* [`SignerP256`](/contracts/latest/api/utils/cryptography#SignerP256): Validates signatures using the secp256r1 curve, common for World Wide Web Consortium (W3C) standards such as FIDO keys, passkeys or secure enclaves.
* [`SignerRSA`](/contracts/latest/api/utils/cryptography#SignerRSA): Verifies signatures of traditional PKI systems and X.509 certificates.
* [`SignerEIP7702`](/contracts/latest/api/utils/cryptography#SignerEIP7702): Checks EOA signatures delegated to this signer using [EIP-7702 authorizations](https://eips.ethereum.org/EIPS/eip-7702#set-code-transaction)
* [`SignerERC7913`](/contracts/latest/api/utils/cryptography#SignerERC7913): Verifies generalized signatures following [ERC-7913](https://eips.ethereum.org/EIPS/eip-7913).
...
* [`MultiSignerERC7913`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913): Allows using multiple ERC-7913 signers with a threshold-based signature verification system.
* [`MultiSignerERC7913Weighted`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913Weighted): Overrides the threshold mechanism of [`MultiSignerERC7913`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913), offering different weights per signer.

<Callout>
Given [`SignerERC7913`](/contracts/latest/api/utils/cryptography#SignerERC7913) provides a generalized standard for signature validation, you don’t need to implement your own [`AbstractSigner`](/contracts/latest/api/utils/cryptography#AbstractSigner) for different signature schemes, consider bringing your own ERC-7913 verifier instead.
</Callout>
...
Suggestively, you can create your own account factory using the [Clones library](/contracts/latest/api/proxy#Clones), taking advantage of decreased deployment costs and account address predictability.
...
Most smart accounts are deployed by a factory, the best practice is to create [minimal clones](/contracts/latest/api/proxy#minimal-clones) of initializable contracts. These signer implementations provide an initializable design by default so that the factory can interact with the account to set it up right after deployment in a single transaction.
...
Regularly, accounts implement [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) to enable smart contract signature verification given its wide adoption. To be compliant means that smart contract exposes an [`isValidSignature(bytes32 hash, bytes memory signature)`](/contracts/latest/api/interfaces#IERC1271-isValidSignature-bytes32-bytes-) method that returns `0x1626ba7e` to identify whether the signature is valid.
...
This is how you enable ERC-1271 using an [`AbstractSigner`](/contracts/latest/api/utils/cryptography#AbstractSigner):
...
<Callout type='warn'>
We recommend using [ERC7739](/contracts/latest/api/utils/cryptography#ERC7739) to avoid replayability across accounts. This defensive rehashing mechanism prevents signatures for this account from being replayed in another account controlled by the same signer. See [ERC-7739 signatures](/contracts/latest/accounts#erc-7739-signatures).
</Callout>

### Batched execution

Batched execution allows accounts to execute multiple calls in a single transaction, which is particularly useful for bundling operations that need to be atomic. This is especially valuable in the context of account abstraction where you want to minimize the number of user operations and associated gas costs. [`ERC-7821`](/contracts/latest/api/account#ERC7821) standard provides a minimal interface for batched execution. 

The library implementation supports a single batch mode (`0x01000000000000000000`) and allows accounts to execute multiple calls atomically. The standard includes access control through the [`_erc7821AuthorizedExecutor`](/contracts/latest/api/account#ERC7821-_erc7821AuthorizedExecutor-address-bytes32-bytes-) function, which by default only allows the contract itself to execute batches.
...
[UserOperations](/contracts/latest/account-abstraction#useroperation) are a powerful abstraction layer that enable more sophisticated transaction capabilities compared to traditional Ethereum transactions. To get started, you’ll need an account, which you can get by [deploying a factory](/contracts/latest/accounts#accounts-factory) for your implementation.
...
<Callout>
Learn more about delegating to an EIP-7702 account in our [EOA Delegation](/contracts/latest/eoa-delegation) section.
</Callout>
...
OpenZeppelin Contracts provides both the building blocks for creating ERC-7579-compliant modules and an [`AccountERC7579`](/contracts/latest/api/account#AccountERC7579) implementation that supports installing and managing these modules. 
