...
ERC-1155 is a novel token standard that aims to take the best from previous standards to create a [**fungibility-agnostic**](/contracts/latest/tokens#different-kinds-of-tokens) and **gas-efficient** [token contract](/contracts/latest/tokens#but-first-coffee-a-primer-on-token-contracts).

<Callout>
ERC-1155 draws ideas from all of [ERC-20](/contracts/latest/erc20), [ERC-721](/contracts/latest/erc721), and [ERC-777](https://eips.ethereum.org/EIPS/eip-777). If you’re unfamiliar with those standards, head to their guides before moving on.
</Callout>

## Multi Token Standard

The distinctive feature of ERC-1155 is that it uses a single smart contract to represent multiple tokens at once. This is why its [`balanceOf`](/contracts/latest/api/token/ERC1155#IERC1155-balanceOf-address-uint256-) function differs from ERC-20’s and ERC-777’s: it has an additional `id` argument for the identifier of the token that you want to query the balance of.

This is similar to how ERC-721 does things, but in that standard a token `id` has no concept of balance: each token is non-fungible and exists or doesn’t. The ERC-721 [`balanceOf`](/contracts/latest/api/token/ERC721#IERC721-balanceOf-address-) function refers to _how many different tokens_ an account has, not how many of each. On the other hand, in ERC-1155 accounts have a distinct balance for each token `id`, and non-fungible tokens are implemented by simply minting a single one of them.
...
Because all state is held in a single contract, it is possible to operate over multiple tokens in a single transaction very efficiently. The standard provides two functions, [`balanceOfBatch`](/contracts/latest/api/token/ERC1155#IERC1155-balanceOfBatch-address---uint256---) and [`safeBatchTransferFrom`](/contracts/latest/api/token/ERC1155#IERC1155-safeBatchTransferFrom-address-address-uint256---uint256---bytes-), that make querying multiple balances and transferring multiple tokens simpler and less gas-intensive.

In the spirit of the standard, we’ve also included batch operations in the non-standard functions, such as [`_mintBatch`](/contracts/latest/api/token/ERC1155#ERC1155-_mintBatch-address-uint256---uint256---bytes-).
...
<Callout>
For an overview of minting mechanisms, check out [Creating ERC-20 Supply](/contracts/latest/erc20-supply).
</Callout>
...
The [`ERC1155`](/contracts/latest/api/token/ERC1155#ERC1155) contract includes the optional extension [`IERC1155MetadataURI`](/contracts/latest/api/token/ERC1155#IERC1155MetadataURI). That’s where the [`uri`](/contracts/latest/api/token/ERC1155#IERC1155MetadataURI-uri-uint256-) function comes from: we use it to retrieve the metadata uri.
...
<Callout>
If you’d like to put all item information on-chain, you can override the `uri()` function in your ERC-1155 contract (or use [`ERC1155URIStorage`](/contracts/latest/api/token/ERC1155#ERC1155URIStorage)) to return a [`Base64`](/contracts/latest/utilities#base64) Data URI with the JSON schema encoded, though it will be rather costly. You could also leverage IPFS to store the URI information, but these techniques are out of the scope of this overview guide
</Callout>

## Sending Tokens to Contracts

A key difference when using [`safeTransferFrom`](/contracts/latest/api/token/ERC1155#IERC1155-safeTransferFrom-address-address-uint256-uint256-bytes-) is that token transfers to other contracts may revert with the following custom error:
