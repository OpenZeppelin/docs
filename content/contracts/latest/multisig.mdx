...
As discussed in the [accounts section](/contracts/latest/accounts#signature-validation), the standard approach for smart contracts to verify signatures is [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271), which defines an `isValidSignature(hash, signature)`. However, it is limited in two important ways:
...
The [`SignerERC7913`](/contracts/latest/api/utils#SignerERC7913) contract allows a single ERC-7913 formatted signer to control an account. The signer is represented as a `bytes` object that concatenates a verifier address and a key: `verifier || key`.
...
The [`MultiSignerERC7913`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913) contract extends this concept to support multiple signers with a threshold-based signature verification system.
...
The contract also provides public functions for querying signer information: [`isSigner(bytes memory signer)`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913-isSigner-bytes-) to check if a given signer is authorized, [`getSigners(uint64 start, uint64 end)`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913-getSigners-uint64-uint64-) to retrieve a paginated list of authorized signers, and [`getSignerCount()`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913-getSignerCount) to get the total number of signers. These functions are useful when validating signatures, implementing customized access control logic, or building user interfaces that need to display signer information.

### MultiSignerERC7913Weighted

For more sophisticated governance structures, the [`MultiSignerERC7913Weighted`](/contracts/latest/api/utils/cryptography#MultiSignerERC7913Weighted) contract extends `MultiSignerERC7913` by assigning different weights to each signer.
