...
OpenZeppelin Contracts has some ``library``s: most are in the [Utils](/contracts/latest/api/utils) directory.
...
For example, imagine you want to change [`AccessControl`](/contracts/latest/api/access#AccessControl) so that [`revokeRole`](/contracts/latest/api/access#AccessControl-revokeRole-bytes32-address-) can no longer be called. This can be achieved using overrides:
...
Here is a modified version of [`AccessControl`](/contracts/latest/api/access#AccessControl) where [`revokeRole`](/contracts/latest/api/access#AccessControl-revokeRole-bytes32-address-) cannot be used to revoke the `DEFAULT_ADMIN_ROLE`:
...
<Callout>
The same rule is implemented and extended in [`AccessControlDefaultAdminRules`](/contracts/latest/api/access#AccessControlDefaultAdminRules), an extension that also adds enforced security measures for the `DEFAULT_ADMIN_ROLE`.
</Callout>
