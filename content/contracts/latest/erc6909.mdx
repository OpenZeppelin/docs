...
<Callout>
To understand the inspiration for a multi token standard, see the [multi token standard](/contracts/latest/erc1155#multi-token-standard) section within the EIP-1155 docs.
</Callout>
...
We’ll use ERC-6909 to track multiple items in a game, each having their own unique attributes. All item types will be minted to the deployer of the contract, which we can later transfer to players. We’ll also use the [`ERC6909Metadata`](/contracts/latest/api/token/ERC6909#ERC6909Metadata) extension to add decimals to our fungible items (the vanilla ERC-6909 implementation does not have decimals).
...
Note that there is no content URI functionality in the base implementation, but the [`ERC6909ContentURI`](/contracts/latest/api/token/ERC6909#ERC6909ContentURI) extension adds it. Additionally, the base implementation does not track total supplies, but the [`ERC6909TokenSupply`](/contracts/latest/api/token/ERC6909#ERC6909TokenSupply) extension tracks the total supply of each token id.
