...
- Use `sorted_map` when each key carries a value, such as price levels, tick registries, leaderboards, or payout vaults. Values may be resources like `Coin<T>`: drain every entry, then call `destroy_empty`.
