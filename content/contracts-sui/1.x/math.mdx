...
- `vector`: in-place sorting macros (`quick_sort!`, `quick_sort_by!`) for unsigned integer vectors and arbitrary types with a custom comparator, plus median selection (`median!` and the precompiled `median_u8` … `median_u256` wrappers) with configurable rounding for even-length input.
