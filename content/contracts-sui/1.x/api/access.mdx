...
<APIGithubLinkHeader
  ...
  link="https://github.com/OpenZeppelin/contracts-sui/blob/af86a0ec4bbbc05b6ec5205b034a4456561c5545/contracts/access/sources/access_control.move"
/>
...
The module supports ordinary role grants and revocations, typed `Auth<Role>` proofs, delayed root-role transfer, delayed root-role renounce, and delayed changes to the root-operation delay. Elapsed delay changes are effective for reads and are cleared by the next delay-refreshing root action.
...
<APIItem
  ...
  kind="struct"
>
Pending change to `default_admin_delay_ms`, including the new delay and the timestamp after which it becomes effective. The pending slot is cleared later by a delay-refreshing entrypoint.
</APIItem>
...
<APIItem
  functionSignature="renounce_role<RootRole, Role>(ac: &mut AccessControl<RootRole>, account: address, ctx: &mut TxContext)"
...
If the pending delay change has already elapsed, this call applies it and clears the pending slot instead of cancelling it.
...
<APIItem
  ...
  kind="public"
>
Returns whether a default-admin delay change is stored as pending.

An elapsed pending change may still return `true` until a delay-refreshing entrypoint clears it. Use `default_admin_delay_ms(ac, clock)` for the effective delay.
</APIItem>
...
An elapsed pending change may still return `some` until a delay-refreshing entrypoint clears it. Use `default_admin_delay_ms(ac, clock)` for the effective delay.
</APIItem>
...
<APIItem
  ...
  kind="public"
>
Returns `some(timestamp_ms)` with the timestamp after which a pending delay change becomes effective. Returns `none` otherwise.
...
<APIItem
  ...
  kind="event"
>
Emitted when a pending root-role transfer or renounce is cancelled, either by `cancel_default_admin_transfer` or because a new pending root action overwrites it.
...
<APIItem
  ...
  kind="error"
>
Raised when cancelling a default-admin delay change but no delay change is pending.
</APIItem>

<APIItem
  functionSignature="EZeroAddress"
...
<APIItem
  functionSignature="EDefaultAdminTransferToSelf"
