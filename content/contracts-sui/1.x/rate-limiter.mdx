...
|---|---|---|
| `try_consume(rl.available(clock), clock)` when empty | Returns `false` (`available()` is `0`, and a zero-unit consume is rejected) — easy to misread as a successful consume | Guard: `let n = rl.available(clock); if (n > 0) { rl.try_consume(n, clock); }` |
