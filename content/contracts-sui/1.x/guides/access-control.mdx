...
After `schedule_after_ms`, `default_admin_delay_ms(ac, clock)` reports the new effective delay. The stored pending slot is cleared later by the next delay-refreshing root action: `begin_default_admin_transfer`, `begin_default_admin_renounce`, `begin_default_admin_delay_change`, or `cancel_default_admin_delay_change`.
...
sequenceDiagram
    ...
    Root->>Registry: later root action(clock)
    Registry-->>Registry: applies elapsed delay and clears pending slot
```
...
The root holder can cancel a pending delay change before `schedule_after_ms`. After `schedule_after_ms`, the same call materializes the elapsed delay change instead of cancelling it:
