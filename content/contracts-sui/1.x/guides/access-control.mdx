...
The root role is intentionally harder to transfer than ordinary roles. It cannot be granted or revoked with `grant_role` and `revoke_role`; use the delayed default-admin transfer flow.
...
- Do not renounce the last root role holder unless the protocol is intentionally being locked.
