...
A faucet needs something to hand out. We use a minimal fixed-supply coin, `RARE_COIN`, that mints its entire supply to the publisher on creation. This part is plain Sui — it does not touch `rate_limiter` — so the rate-limiting logic stays the focus.
