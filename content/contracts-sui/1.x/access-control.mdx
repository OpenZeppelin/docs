...
public fun assert_operator(access: &AccessControl<ROLES>, ctx: &mut TxContext) {
    access.assert_has_role<_, OperatorRole>(ctx.sender());
}
```
...
Use `assert_has_role` when you are retrofitting an existing public function and changing the signature would be disruptive or invalid for package-upgrade compatibility.
